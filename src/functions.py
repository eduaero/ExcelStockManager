...
def downloadOneProduct(product,wcapi,toCsv=1,folder='OneProduct'):
    ...
    if (1):  # wp-json/wc/v3/products/<id>/variations
...
def changeProdFeatureFromDB(wcapi,writeTable,varTable,varWC):
    lenDb = writeTable.__len__()
    for id in range(1,lenDb + 1):
        while True:
          try:
            ...
            idProd=row['prodID']
            value=""
            # newPrice = '%4.2f' % row[varTable]
            # print('Contador: %d Producto: %s   Antiguo Precio: %s Nuevo Precio: %s' %(id, idProd, row['regular_price'], newPrice))
            print('Contador: %d Producto: %s   Fb visibility: %s ' % (id, idProd, value,))
            # data = {regular_price:newPrice}
            data={}
            data['meta_data'] = []
            data['meta_data'].append({'key': "fb_visibility", 'position': x})
...
def downloadAllProducts(wcapi,nameDataBase):
    ...
    while True:
        ...
        file_json = json.loads(file)
        for x in range(0,file_json.__len__()):
            dataDb=dict(prodName=file_json[x]['name'], prodID=file_json[x]['id'], regular_price=file_json[x]['regular_price'])
